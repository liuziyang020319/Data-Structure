...
int main(){
    ...
    if(flag==1){
        ...
        for(i=1;i<=n;++i){
            ...
            char filename[100];
            sprintf(filename,"png/tree%d.dot",i);
            FILE *fp = fopen(filename, "w");
            showTree(root,fp);   
            fclose(fp);
            char cmdname[100];
            sprintf(cmdname,"dot -Tpng png/tree%d.dot -o png/tree%d.png > out.txt 2>&1",i,i);
            system(cmdname);   
            sprintf(filename,"rm png/tree%d.dot",i);
            system(filename);              
        }
        system("python Make.py ./png");
        system("C:/Users/lzy/Desktop/课堂展示/AVL/png/result.gif");
    }
